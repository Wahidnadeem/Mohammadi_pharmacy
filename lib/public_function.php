...
function clean_data($date,$exp_type = '-'){
	...
	function jalali_to_gregorian_date($date, $exp_type)
	{
 ...
 */
      ...
      function getPendingStatusNumber($db){
        ...
        function getOption($table , $id = ''  , $name , $order = 'DESC'){
            global $db;

            $query = $db->query("SELECT * FROM $table WHERE deleted = '0' ORDER BY id $order");
            echo '<option> یکی از گزینه های ذیل را  انتخاب کنید  </option>';
