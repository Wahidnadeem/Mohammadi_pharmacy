...
if(isset($_POST['insert'])){
    ...
    foreach ($_POST['durg_ides'] as $key => $value) {
        ...
        $expire_dates       = VD($_POST['expire_dates'][$key]);
...
if (isset($_GET['id']) AND isset($_GET['delete'])){
    ...
    $customers_data = $db->query("SELECT * FROM `customer_transaction` WHERE `customer_id` = '$customer_iddd' AND `type` = 'debt' limit 1")->fetch();
    $customer_amounts = $customers_data['amount'];

    $totlas_amount = $customer_amounts - $buy_sell_amounts;

    $update_customer_transcation = $db->query("UPDATE `customer_transaction` SET `amount` = '$totlas_amount' WHERE `customer_id` = '$customer_iddd' AND `type` = 'debt'"); 
...
// delete query assets
if (isset($_GET['id']) AND isset($_GET['buy_factor_assets'])){

    $id    = base64_decode($_GET['id']);

    $select_buy_sell_assets_row = $db->query("SELECT * FROM `buy_sell_assets` WHERE `id` = '$id' AND `deleted` = '0' AND `type` = 'buy'");
...
if (isset($_POST['edit'])) {
    ...
    $remain             = (!empty(VD($_POST['remain'])) ? VD($_POST['remain']) *- 1 : 0 );
    $current_remain     = ( !empty(VD($_POST['current_remain'])) ? VD($_POST['current_remain']) : 0 );
    $customer_id        = VD($_POST['customer_id']);
    $payment_amount     = VD($_POST['payment_amount']);
    $total_price        = VD($_POST['total_price']);


    $transaction_data = $db->query("SELECT * FROM `transaction` WHERE buy_sell_id = '$id' ORDER BY id DESC LIMIT 1 ");
    
    if($transaction_data->rowCount() > 0){
        
        $transaction_row = $transaction_data->fetch();
        $edit = edit('transaction',['amount' => $payment_amount ] , $transaction_row['id']);

        $customer_transaction_data = $db->query("SELECT * FROM customer_transaction WHERE customer_id = $customer_id LIMIT 1 ");
        $customer_transaction_row = $customer_transaction_data->fetch();
        echo $remain;
        $final_amount = $customer_transaction_row['amount'] + ($current_remain * 1 ) + $remain;
        
        $edit = edit('customer_transaction',[ 'amount' => $final_amount ],$customer_transaction_row['id']);

    }

    // $select_row_buy_sell = $db->query("SELECT * FROM `buy_sell_drug` WHERE `id` = '$id' AND `deleted` = '0' AND `type` = 'buy' LIMIT 1")->fetch();
    // $buy_sell_amount =  $select_row_buy_sell['remain'];
    // $buy_sell_customer_id = $select_row_buy_sell['customer_id']; 


    // $customer_transction_old = $db->query("SELECT * FROM `customer_transaction` WHERE `customer_id` = '$buy_sell_customer_id' AND `type` = 'debt' LIMIT 1")->fetch();
    // $amount_customer_transcation = $customer_transction_old['amount'];
    // $customer_transaction_id = $customer_transction_old['customer_id'];

    // $customer_transction = $db->query("SELECT * FROM `customer_transaction` WHERE `customer_id` = '$customer_id' AND `type` = 'debt' LIMIT 1");
...
if (isset($_POST['edit_assets'])) {
    ...
    if (!empty($_POST['expire_date'])) {
        ...
        $select_buy_sell_assets_row = $db->query("SELECT * FROM `buy_sell_assets` WHERE `id` = '$id' AND `deleted` = '0' AND `type` = 'buy' LIMIT 1");
            if ($select_buy_sell_assets_row->rowCount() > 0) {
                 $buy_sell_assets_data    = $select_buy_sell_assets_row->fetch();
                 $amount_buy_sell_assets  = $buy_sell_assets_data['amount'];
