<?php
  ...
  $mounts = $db->query("SELECT * FROM fix WHERE `name` = 'expire_date' LIMIT 1 ")->fetch()['value'];
  $expiry_date =  date('Y-m-d',strtotime("$mounts months"));

  $view_data = $db->prepare("SELECT * FROM `buy_sell_assets` WHERE type = 'buy' AND deleted = 0 AND status != 'done' and  `expire_date` < '$expiry_date' $condition ORDER BY id DESC");
  $view_data->execute();
        ...
        <div id="wrapper" class="wrapper animsition">
            ...
            <div id="page-wrapper">
                ...
                <div class="content">
                   ...
                   <a id="export_button" href="report_expiry_date.php" class="btnc btnc-primary btnc-icon-anim pointer" style="text-decoration: none; bottom: 12px;" target="_blank" >
                        <i class="fa fa-file-excel-o"></i>&nbsp;
                        <span class="ladda-label bfont">خروجی اکسل</span>&nbsp;
                    </a>
                    &nbsp;
                     <a href="report_expiry_date.php" class="btnc btnc-success btnc-icon-anim pointer" style="text-decoration: none; bottom: 12px;">
                        <i class="fa fa-rotate-left"></i>&nbsp;
                        <span class="ladda-label bfont">برگشت  به  لیست تاریخ انقضا دوا ها</span>&nbsp;
                    </a>
