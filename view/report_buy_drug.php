<?php
   ...
   if(!empty($_POST['search'])){
        ...
        if(!empty($_POST['code_number'])){
            ...
            $buy_sell_id = $db->query("SELECT * FROM `buy_sell_drug` WHERE `code_number` = $code_number and `type` = 'buy' AND deleted = 0 $start_end_date LIMIT 1 ");
        ...
        if(!empty($_POST['customer_id'])){
            ...
            $buy_sell_ides = $db->query("SELECT id FROM `buy_sell_drug` WHERE `customer_id` = $customer_id and `type` = 'buy' AND deleted = 0 $start_end_date ");
        ...
        if(!empty($_POST['start_date']) && !empty($_POST['end_date']) && empty($_POST['code_number']) && empty($_POST['customer_id']) ){
            ...
            $buy_sell_ides = $db->query("SELECT id FROM `buy_sell_drug` WHERE `type` = 'buy' AND deleted = 0 AND `date` BETWEEN  '$start_date' AND '$end_date' ");
   ...
   $view_data = $db->prepare("SELECT * FROM `buy_sell_assets` WHERE type = 'buy' AND deleted = 0 AND status != 'done' $condition ORDER BY id DESC LIMIT $to OFFSET $from");
   $view_data->execute();
   
   $list_data  = $db->query("SELECT count(id) as record FROM `buy_sell_assets` WHERE type = 'buy' AND deleted = 0 AND status != 'done' $condition ")->fetch();
...
<html lang="en">
   ...
   <body class="body" style="scrollbar-width: thin;">
      <div id="wrapper" class="wrapper animsition">
         ...
         <!-- /.Navigation -->
         <?php $menu = "report"; $sub = "report_buy_drug";?>
         <!-- Sidebar -->
         ...
         <div id="page-wrapper">
            ...
            <div class="content">
               ...
               <div class="row">
                        ...
                        <div class="panel-body bfont">
                           ...
                           <form id="validation-form" class="form-inline form-label-left" method="post" action="">
                            ...
                            <div class="row"> 
                                ...
                                <div class="item form-group col-md-2">
                                    <label class="control-label " for="customer_id">  نام فروشنده دوا</label>
                                    <div >
                ...
                <a href="excel_buy_drug.php?condition=<?php echo $condition;?>" class="btnc btnc-primary btnc-icon-anim pointer" style="text-decoration: none; bottom: 12px;" target="_blank" >
               ...
               <div class="panel panel-default lobidisable">
                  <div class="panel-heading">
                     <div class="panel-title">
                        <h4 class="gray bold bfont"><i class="fa fa-inbox"></i> لیست دوا های خرید شده </h4>
                     </div>
                  </div>
                                  ...
                                  foreach ($view_data as $key => $row) {
                                      ...
                                      echo '
                                          <tr style="height:40px;">
                                              ...
                                              <td> <p  class ="drug-code cfont"> '. $drug_row['drug_code'].' </p> </td>
